...
```{r}
div_winners_id <- team_data %>% 
  dplyr::select(-lgID,-Rank,-franchID,-WCWin,-LgWin,-WSWin,-name) %>% 
  dplyr::select(-contains("team")) %>% 
  dplyr::select(-contains("park"))

set.seed(123)
training.samples_2 <- div_winners_id$divID %>%
  createDataPartition(p = 0.8, list = FALSE)
train.data_id  <- div_winners_id[training.samples_2,]
test.data_id <- div_winners_id[-training.samples_2,]
...
roctrain_id_c <-roc(response=train.data_id$divID=="C",predictor=div_win_predict_train,plot=TRUE,main="ROC Curve for DivWin",auc=TRUE)
roctest_id_c<- roc(response=test.data_id$divID=="C",predictor=div_win_predict_test,plot=TRUE,auc=TRUE,add=TRUE,col=2)
legend(0,0.4,legend=c("training","testing"),fill=1:2)


youden_id_train_c<-coords(roctrain_id_c,"b",best.method="youden",transpose=TRUE)
youden_id_train_c
youden_id_train_c[2]+youden_id_train_c[3]

youden_id_test_c<-coords(roctest_id_c,x=0.01842452 ,transpose=TRUE)
youden_sens_spec_c <- youden_id_test_c[2]+youden_id_test_c[3]
youden_sens_spec_c
...
roctrain_id_e <-roc(response=train.data_id$divID=="E",predictor=div_win_predict_train,plot=TRUE,main="ROC Curve for DivWin",auc=TRUE)
roctest_id_e<- roc(response=test.data_id$divID=="E",predictor=div_win_predict_test,plot=TRUE,auc=TRUE,add=TRUE,col=2)
legend(0,0.4,legend=c("training","testing"),fill=1:2)


youden_id_train_e<-coords(roctrain_id_e,"b",best.method="youden",transpose=TRUE)
youden_id_train_e
youden_id_train_e[2]+youden_id_train_e[3]

youden_id_test_e<-coords(roctest_id_e,x=0.3821811,transpose=TRUE)
youden_sens_spec_e <- youden_id_test_e[2]+youden_id_test_e[3]
youden_sens_spec_e
...
*Team Division W*
```{r}
roctrain_id_w <-roc(response=train.data_id$divID=="W",predictor=div_win_predict_train,plot=TRUE,main="ROC Curve for DivWin",auc=TRUE)
roctest_id_w<- roc(response=test.data_id$divID=="W",predictor=div_win_predict_test,plot=TRUE,auc=TRUE,add=TRUE,col=2)
legend(0,0.4,legend=c("training","testing"),fill=1:2)


youden_id_train_w<-coords(roctrain_id_w,"b",best.method="youden",transpose=TRUE)
youden_id_train_w
youden_id_train_w[2]+youden_id_train_w[3]

youden_id_test_w<-coords(roctest_id_w,x= 0.03102454,transpose=TRUE)
youden_sens_spec_w <- youden_id_test_w[2]+youden_id_test_w[3]
youden_sens_spec_w
```

```{r}
divisions <- c("C", "E", "W")
sens_and_spec_sum <- c(youden_sens_spec_c,youden_sens_spec_e,youden_sens_spec_w)
table_for_bar_graph <- as_tibble(data.frame(divisions,sens_and_spec_sum))
table_for_bar_graph
...
```{r}
table_for_bar_graph %>% 
  ggplot(mapping = aes(x = divisions,y = sens_and_spec_sum))+
  geom_col()+
  labs(x = "Team divisions",y = "Sum of sensitivity and specificty",
       title = "Sum of sensitivity and specificity across team divisions")+
  theme_classic()
```
**The sum of sensitivity and specificity across team divisions varied, it was poorest for team division E (0.9714588) this was less than 1 (value we get for a free model), for team division C it was better with the sum coming to 1.0804922 implying that in this division there was a 8% increase in numbers of correct classification. The model performed best on team division W with the sum of sensitivity and specificity coming to 1.1648352 implying that there was a 16% overall increase in numbers of correct classification.The model made the most classification on team division W, followed by team division C and the model didn't perform so well with team division E**